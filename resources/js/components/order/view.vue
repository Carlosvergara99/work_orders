  ...
  <div>
    ...
    <v-app id="inspire">
      ...
      <v-dialog v-model="dialog5" max-width="600px">
        <v-card>
          <v-card-title> Editar Orden </v-card-title>
          <v-card-text>
            <v-container>
              <v-row>
                <v-col cols="6">
          ...
          <v-card-actions>
            ...
            <v-btn color="primary darken-1" text @click="close"> Cerrar </v-btn>
...
export default {
  ...
  methods: {
    getsupplement() {
      axios.get("api/order/get/supplement")
        .then((response) => {
    ...
    getorder() {
      axios.post("api/order/get?page=" + this.pagination.current)
        .then((response) => {
    ...
    update() {
      const data = {
        ...
        id_order: this.from.id_order,
        status_id: this.from.status_id,
      };
      if (!Number.parseInt(data.value_work) ) {
        toast.fire({
                 icon: 'error',title: 'el campo valor del trabajo es obligatorio y debe ser numerico'
         })
 
       }else if(data.status_id ==''){

        toast.fire({
                 icon: 'error',title: 'el campo estado del trabajo es obligatorio'
         })
      }else if(data.date_execution == ''){
         toast.fire({
                 icon: 'error',title: 'el campo fecha de asignacion es obligatorio '
           })

      }else{
        axios.post("api/order/update", data)
        .then((response) => {
          this.getorder();
          toast.fire({
